...
module.exports = function (platform) {
  ...
  return {
    ...
    fetch: function (url, repo, opts, callback) {
      var url = urlParse(url);
      opts.pathname = url.pathname;
      opts.hostname = url.hostname;
      opts.agent = extract(platform, "agent");
      return runMachine(url, function (write, emit) {
        return fetchMachine(opts, repo, write, emit);
      }, callback);
    },
    push: function (url, repo, opts, callback) {
      var url = urlParse(url);
      opts.pathname = url.pathname;
      opts.hostname = url.hostname;
      opts.agent = extract(platform, "agent");
      return runMachine(url, function (write, emit) {
        return pushMachine(opts, repo, write, emit);
      }, callback);
    }
  };
...
function fetchMachine(opts, repo, write, emit) {
  ...
  return refsMachine(write, function (result, serverCaps) {
    ...
    if (serverCaps["include-tag"]) caps.push("include-tag");

...
function repo(db) {
  ...
  function unpack(raw, opts, callback) {
    ...
    function onRead(err, item) {
      if (item === undefined) {
        if (opts.onProgress) opts.onProgress(formatProgress() + "\n");
        return callback(err);
      }
      if (!bops.is(item)) {
        if (head) return save(item);
        return start(item)
      }
      parts.push(item);
      packStream.read(onRead);
    ...
    function start(item) {
      ...
      count++;
      var percent = Math.round(count / num * 100);
      if (opts.onProgress) opts.onProgress(formatProgress() + "\r");
      packStream.read(onRead);
    }

    function save(next) {
      var object = head;
      object.body = bops.join(parts);

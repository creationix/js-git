...
run([
  ...
  function testPackFrame(end) {
    ...
    function next() {
      ...
      // Skip delta frames
      if (item.type.indexOf("-") > 0) return next();

      codec.packFrame(item.type, item.body, function (err, packed) {
        if (err) return end(err);
        console.log(packed.length, item.body.length);
        next();
      })
    }
  }
